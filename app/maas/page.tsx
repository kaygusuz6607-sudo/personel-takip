...
export default function MaasTahakkukPage() {
  ...
  // Elden / Banka Dağılımını Hızlı Düzenleme Modalı
  const [eldenModalStaff, setEldenModalStaff] = useState<PayrollRow | null>(null);
  const [manualEldenValue, setManualEldenValue] = useState<number | string>("");
  const [savingElden, setSavingElden] = useState(false);

  const openEldenModal = (row: PayrollRow) => {
    setEldenModalStaff(row);
    setManualEldenValue(row.payroll.unofficialAmount ?? 0);
  };

  const handleSaveEldenManual = async () => {
    if (!eldenModalStaff) return;
    try {
      setSavingElden(true);
      ...
      const payload = {
        ...eldenModalStaff.payroll,
        staffId: eldenModalStaff.staffId,
        year,
        month,
      ...
      alert("Elden tutarı kaydedilemedi.");
    } finally {
      setSavingElden(false);
    }
  };

  const handleResetToAutoElden = (row: PayrollRow) => {
    const calc = calculatePayroll({
      ...
      dailyWorkDays: Number(row.payroll.dailyWorkDays) || 0,
      holidayWorkDays: Number(row.payroll.holidayWorkDays) || 0,
      holidayChoice: row.payroll.holidayChoice || "LEAVE_1_TO_1",
      bonusAmount: Number(row.payroll.bonusAmount) || 0,
      bonusDescription: row.payroll.bonusDescription || "",
      deductionAmount: Number(row.payroll.deductionAmount) || 0,
      deductionDescription: row.payroll.deductionDescription || "",
      isManualTax: row.payroll.isManualTax,
      manualSgkEmployee: row.payroll.sgkEmployee,
      manualUnemployment: row.payroll.unemploymentEmployee,
      manualIncomeTax: row.payroll.incomeTax,
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full space-y-6">
                      ...
                      <td className="py-3 px-3 text-right min-w-[150px]">
                        ...
                        <div className="flex items-center justify-end gap-1 mt-1">
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[10px] font-semibold text-amber-900 bg-amber-50 hover:bg-amber-200/80 border border-amber-300 rounded transition-colors shadow-2xs"
              ...
              <div className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-3 items-center">
                ...
                <div className="p-3 bg-amber-50/80 rounded-xl border border-amber-200 space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => handleResetToAutoElden(activeModalStaff)}
                      className="text-[10px] font-bold text-teal-800 hover:text-teal-950 bg-teal-100/90 hover:bg-teal-200 px-2 py-0.5 rounded-md transition-colors inline-flex items-center gap-1"
                      title="Otomatik atama hesabına sıfırla"
                    >
      ...
      {/* Elden / Banka Dağılımını Manuel Düzenleme Modalı */}
      {eldenModalStaff && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl p-6 relative border border-slate-100 animate-in fade-in zoom-in duration-200 space-y-4">
            {/* Başlık */}
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 text-sm">
                    Elden / Banka Ödeme Dağılımı
                  </h3>
                  <p className="text-xs text-slate-500 font-medium">
                    {eldenModalStaff.fullName} ({eldenModalStaff.title || "Personel"})
                  </p>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setEldenModalStaff(null)}
            ...
            {/* Toplam Net Maaş Bilgisi */}
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200/80 flex items-center justify-between text-xs">
              <span className="text-slate-600 font-medium">Toplam Net Ödeme:</span>
              <span className="font-extrabold text-teal-800 text-base">
                {formatCurrency(eldenModalStaff.payroll.netTotal)}
              </span>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input
                  ...
                  step="0.01"
                  min="0"
                  max={eldenModalStaff.payroll.netTotal}
                  value={manualEldenValue}
                  onChange={(e) => setManualEldenValue(e.target.value)}
                  placeholder="0.00"
                  className="w-full px-3.5 py-2.5 bg-amber-50/50 border border-amber-300 rounded-xl text-lg font-black text-amber-950 focus:outline-none focus:ring-2 focus:ring-amber-500/20"
                />
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 pt-1">
                <button
                  type="button"
                  onClick={() => handleResetToAutoElden(eldenModalStaff)}
                  className="inline-flex items-center gap-1 px-2.5 py-1 bg-teal-50 hover:bg-teal-100 text-teal-800 border border-teal-200 rounded-lg text-xs font-semibold transition-colors"
                  title="Atama tarihine göre otomatik hesaplanan tutara dön"
                >
                  <RotateCcw className="w-3 h-3 text-teal-600" />
                  <span>Otomatik Hesaba Dön</span>
                </button>

                <button
                  type="button"
                  onClick={() => setManualEldenValue(0)}
                ...
                <button
                  type="button"
                  onClick={() => setManualEldenValue(eldenModalStaff.payroll.netTotal)}
                  className="px-2.5 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-medium transition-colors"
                >
                  Tümü Elden
                </button>
            ...
            {/* Dağılım Özeti Kartı */}
            {(() => {
              const eldenVal = Math.max(0, Math.min(eldenModalStaff.payroll.netTotal, Number(manualEldenValue) || 0));
              const bankaVal = Number((eldenModalStaff.payroll.netTotal - eldenVal).toFixed(2));
              return (
                <div className="p-3 bg-gradient-to-r from-amber-50/80 to-blue-50/80 rounded-xl border border-slate-200 text-xs space-y-1.5">
                  <div className="flex items-center justify-between font-bold text-amber-950">
                    <span>💵 Elden Ödenecek (Nakit):</span>
                    <span className="text-sm font-black text-amber-900">{formatCurrency(eldenVal)}</span>
                  </div>
                  <div className="flex items-center justify-between font-bold text-blue-950 pt-1 border-t border-slate-200/60">
                    <span>🏦 Bankadan Yatacak (Resmî):</span>
                    <span className="text-sm font-black text-blue-900">{formatCurrency(bankaVal)}</span>
                  </div>
                </div>
              );
            })()}
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
              ...
              <button
                type="button"
                onClick={handleSaveEldenManual}
                disabled={savingElden}
                className="px-4 py-2 text-xs font-bold text-white bg-teal-700 hover:bg-teal-800 rounded-xl shadow-xs transition-colors flex items-center gap-1.5"
              >
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-300" />
                <span>{savingElden ? "Kaydediliyor..." : "Kaydet ve Uygula"}</span>
              </button>
            </div>
          </div>
        </div>
